...
/// A struct to snap polyline normals to axis-aligned directions
#[derive(Debug, Clone)]
pub struct SnapNormal;

impl NormalConstraints for SnapNormal {
    fn project_local_normal_mut(&self, normal: &mut Vector<Real>) -> bool {
        simple_normal_snapper(normal);
        true
    }
}

/// Snaps a normal to the nearest axis-aligned direction (0, ±1) or (±1, 0)
pub fn simple_normal_snapper(normal: &mut Vector<Real>) {
...
impl SimdCompositeShape for Polyline {
    fn map_part_at(
        ...
        let snap_normal = SnapNormal;
        f(None, &tri, Some(&snap_normal))
    }
...
impl TypedSimdCompositeShape for Polyline {
    ...
    type PartNormalConstraints = SnapNormal;
    type PartId = u32;
